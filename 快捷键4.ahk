...
;动态创建web2依赖
; 注意：#Include 在加载期执行，运行期的 FileExist/init 来不及先写出文件。
; 使用 *i：Temp 被清理导致文件缺失时不中断加载，让下方逻辑写出依赖后再重启。
if (!FileExist(A_Temp "\WebView2.ahk") || !FileExist(A_Temp "\WebView2Loader.dll")) {
    outDependData.init()     ;0.初始化外部依赖数据
    Run(A_AhkPath ' "' A_ScriptFullPath '"')
    ExitApp()
}
#Include *i %A_Temp%\WebView2.ahk
...
ak.restoreCursors()      ;⑥启动时清掉可能残留的系统忙碌光标
;.........................................
...
class outDependData {
    ...
    ;从脚本源文件提取 varName:="..." / varName.="..." 拼接值（避免超长单行触发 Declaration too long）
    static _readScriptVar(varName) {
        content := FileRead(A_ScriptFullPath)
        ; 匹配 := "..." 后可选若干 .="..."
        if !RegExMatch(content, varName '\s*:=\s*"([^"]*)"((?:\s*' varName '\s*\.=\s*"[^"]*")*)', &m)
            return ""
        result := m[1]
        pos := 1
        while RegExMatch(m[2], varName '\s*\.=\s*"([^"]*)"', &p, pos) {
            result .= p[1]
            pos := p.Pos + p.Len
        }
        return result
    ...
    ;将文件原始字节编码为 base64，直接复用 ak.getBase64
    static _encodeFileToBase64(path) {
        return ak.getBase64(path)
    }

    ;把超长 base64 拆成多行 := / .= ，避免 AHK “Declaration too long”
    static _formatB64Decl(varName, b64, chunkSize := 8000) {
        b64 := String(b64)
        if b64 = ""
            return varName ':=""'
        out := ""
    ...
    ;用正则定位匹配范围，再做字符串整体替换，更新脚本末尾的 varName 声明块
    static _updateScriptVar(varName, newB64) {
        path       := A_ScriptFullPath
        content    := FileRead(path)
        replacement := outDependData._formatB64Decl(varName, newB64)
        ; 兼容旧单行，以及 := + 若干 .= 的多行块
        newContent := RegExReplace(content
            , varName '\s*:=\s*"[^"]*"(?:\s*' varName '\s*\.=\s*"[^"]*")*'
            , replacement)
...
;     static snipastePath:="Snipaste-2.5.6-Beta-x64\Snipaste.exe"
     ...
     static initAll()
     {
        ...
        TemplateEngine.regesterHotKey()   ;注册热键
...
;       setTimer ()=>this.setHostsByNetTimer(),-1 ;配置远程主机的ipv6本地hosts域名
     ...
     static RunExtraScripts() {
         ...
         ; 先清掉同名扩展脚本（含 helpme 另一路径的旧实例），避免双 UI
         this.KillExtraScripts()
         this.KillExtraScriptsByBaseName(scriptDir)
         successCount := 0
     ...
     ;关闭 主脚本打开的ahk_ext下的子脚本（当前目录 + helpme 镜像目录）
     static KillExtraScripts(ExitReason := 0, ExitCode := 0) {
         dirs := [A_ScriptDir . "\ahk_ext"]
         try {
             if init.helpme2Path && init.helpme2Path != A_ScriptDir
                 dirs.Push(init.helpme2Path . "\ahk_ext")
         }
         oldDetect := DetectHiddenWindows(true)
         for scriptDir in dirs {
             if !DirExist(scriptDir)
                 continue
             Loop Files scriptDir "\*.ahk", "F" {
         ...
         ; 再按文件名兜底，防止路径不一致残留
         this.KillExtraScriptsByBaseName(A_ScriptDir . "\ahk_ext")
         return 0
     }
     ; 按脚本文件名杀掉所有 AutoHotkey 实例（不同盘符/路径也会命中）
     static KillExtraScriptsByBaseName(scriptDir := "") {
         if scriptDir = ""
             scriptDir := A_ScriptDir . "\ahk_ext"
         if !DirExist(scriptDir)
             return
         names := []
         Loop Files scriptDir "\*.ahk", "F"
             names.Push(StrLower(A_LoopFileName))
         if !names.Length
             return
         try {
             for proc in ComObjGet("winmgmts:").ExecQuery(
                 "Select ProcessId,CommandLine from Win32_Process where Name='AutoHotkey64.exe' or Name='AutoHotkey.exe' or Name='AutoHotkey32.exe'") {
                 cmdLine := ""
                 try cmdLine := proc.CommandLine
                 if !cmdLine
                     continue
                 cmd := StrLower(String(cmdLine))
                 for name in names {
                     if InStr(cmd, "\" name) || InStr(cmd, "/" name) || InStr(cmd, '"' name '"') {
             ...
             OutputDebug("KillExtraScriptsByBaseName: " e.Message "`n")
         }
     }
}
;[@init-B42A9039F3BF47D8B038FEF62002708B]
...
class TemplateEngine {
    static recentInput := ""
    static watchIh := ""
    ; 一旦打出 ?? / ？？，粘住屏蔽 ?xxx 模板，避免回车清空缓冲后仍误触发（与剪贴板 ?? 搜索冲突）
    static blockQuestionTemplates := false
    ...
    static regesterHotKey() {
        ...
        for key, val in sysconfig {
            this.RegisterTemplateHotstring(key, val)
            ; 中英文问号兼容
            if InStr(key, "?") {
                zhKey := StrReplace(key, "?", "？", , , 1)
                this.RegisterTemplateHotstring(zhKey, val)
            }
        }
    }

    ; 后台只盯问号键（不再常驻 InputHook 监听每个字符——会拖慢所有输入框）
    ; 只盯 ASCII ?：剪贴板搜索是 ??；中文 ？ 热串会打断 :?B0:？prod 等模板匹配
    static StartInputWatch() {
        if this.watchIh = "hotkeys"
            return
        try Hotkey("~*?", (*) => TemplateEngine.OnQuestionMark("?"), "On")
        try Hotkey("~*Backspace", (*) => TemplateEngine.OnWatchKeyDown(0x08), "On")
        try Hotkey("~*Enter", (*) => TemplateEngine.OnWatchKeyDown(0x0D), "On")
        try Hotkey("~*Esc", (*) => TemplateEngine.OnWatchKeyDown(0x1B), "On")
        this.watchIh := "hotkeys"
    }

    static OnQuestionMark(ch := "?") {
        this.recentInput .= ch
        if StrLen(this.recentInput) > 32
            this.recentInput := SubStr(this.recentInput, -31)
        if InStr(this.recentInput, "??") || InStr(this.recentInput, "？？")
            this.ArmDoubleQuestionBlock()
    }

    static OnWatchKeyDown(vk) {
        ; Backspace：无缓冲时立刻返回，避免每次退格都做字符串操作
        if vk = 0x08 {
            if this.recentInput = ""
                return
            this.recentInput := SubStr(this.recentInput, 1, -1)
            return
        }
        ; Enter / Escape：清空滚动缓冲，但双问号屏蔽要多留一会；并 Reset 挂起的 ?prod
        if vk = 0x0D || vk = 0x1B {
            if this.blockQuestionTemplates || InStr(this.recentInput, "??") || InStr(this.recentInput, "？？") {
                this.ArmDoubleQuestionBlock()
                SetTimer(() => TemplateEngine.ClearDoubleQuestionBlock(), -2000)
            }
            this.recentInput := ""
            if vk = 0x1B
                this.ClearDoubleQuestionBlock()
        }
    }

    static ArmDoubleQuestionBlock() {
        this.blockQuestionTemplates := true
        try Hotstring("Reset")
    }

    static ClearDoubleQuestionBlock() {
        this.blockQuestionTemplates := false
    }

    /**
     * 若最近输入里出现 ?? / ？？，且当前触发词以 ?/？ 开头，则视为剪贴板 ?? 搜索，不展开模板。
     * 例：打了 ??prod 时，触发词 ?prod 前面还有一个 ?，应跳过。
     */
    static ShouldSkipForDoubleQuestion(triggerWord := "") {
        t := String(triggerWord)
        first := t != "" ? SubStr(t, 1, 1) : ""
        ; 非 ? 开头的模板不受双问号屏蔽影响
        if t != "" && first != "?" && first != "？"
            return false
        if this.blockQuestionTemplates
            return true
        r := this.recentInput
        if r = ""
            return false
        ...
        if t = ""
            return false
        ; recent 以 trigger 结尾时，再看 trigger 前一个字符是不是问号
        if StrLen(r) >= StrLen(t) + 1 && SubStr(r, StrLen(r) - StrLen(t) + 1) = t {
            before := SubStr(r, StrLen(r) - StrLen(t), 1)
            if before = "?" || before = "？"
                return true
        }
        return false
    ...
    ; 处理 带变量的热字符串 和普通字符串
    static RegisterTemplateHotstring(key, val) {
        if InStr(Trim(key), "::") != 1
            return

        rawTrigger := SubStr(Trim(key), 3) ; 剥离前缀 "::"

        ; 正则匹配：提取触发词（如 ?name）和 参数列表（如 |$1|$2...）
        if RegExMatch(rawTrigger, "^([^|]+)(\|.*)$", &m) {
            triggerWord := m[1]   ; 得到 "?name"

            ; 【核心修正】加入 * 选项，只要打出 ?name 就立刻启动 InputHook 在后台搜集 |123|123
            Hotstring(":B0?*:" triggerWord, (*) => this.HandleMultiVarTemplate(triggerWord, val))
        } else {
            ; B0：跳过时不自动删字；真正展开时再手动退格触发词+结束符
            Hotstring(":?B0:" rawTrigger, this.hotKeyHandler(val, rawTrigger))
        }
    }

    ; 处理无变量热键（生成闭包，纯 SendText 输出）
    ; AHK v2 的 => 不能带 { } 块，改用嵌套函数捕获局部变量
    static hotKeyHandler(content, triggerWord := "") {
        content := StrReplace(content, "\n", "`n")
        localContent := content
        localTrigger := triggerWord
        handler(*) {
            if TemplateEngine.ShouldSkipForDoubleQuestion(localTrigger) {
                try Hotstring("Reset")
                return
            }
            ; B0 下需自己擦掉触发词 + 结束符（空格/回车等）
            bs := StrLen(localTrigger) + 1
            Send("{BS " bs "}")
            Sleep(20)
            SendText(localContent)
        }
        return handler
    }

    ; 多变量捕获与替换处理
    static HandleMultiVarTemplate(triggerWord, rawVal) {
        if this.ShouldSkipForDoubleQuestion(triggerWord) {
            try Hotstring("Reset")
            return
        }
        ; 创建 InputHook 监听器，监听空格、Enter、Tab 作为结束触发信号
        ih := InputHook("V", "{Space}{Enter}{Tab}")

        ; 监听打字结束事件
        ih.OnEnd := (ihInst) => this.ProcessTemplateOutput(ihInst, triggerWord, rawVal)

        ih.Start()
    }

    ; 异步处理回调输出
    static ProcessTemplateOutput(ih, triggerWord, rawVal) {
        if this.ShouldSkipForDoubleQuestion(triggerWord) {
            try Hotstring("Reset")
            return
        }
        userInput := ih.Input ; 获取用户打出 ?name 之后的后续内容，例如 "|123|123"

        ; 【安全校验】如果后续输入的第一个字符不是 '|'，说明用户只是普通打出了 ?name，不做任何替换
        if (userInput == "" || SubStr(userInput, 1, 1) != "|")
            return

        ; 计算退格总长度（触发词 ?name + 用户打出的参数 |123|123 + 结尾空格/回车）
        endCharLen := (ih.EndReason == "EndKey") ? 1 : 0
        totalBackspaces := StrLen(triggerWord) + StrLen(userInput) + endCharLen

        ; 1. 剥离开头的 |，然后按 | 拆分为数组
        cleanInput := SubStr(userInput, 2)
        paramList := StrSplit(cleanInput, "|")

        ; 2. 依次替换 $1, $2, $3 ... $N
        finalResult := rawVal
        for index, paramValue in paramList {
            placeholder := "$" index
            finalResult := StrReplace(finalResult, placeholder, paramValue)
        }

        ; 3. 擦除屏幕上用户打出的原始文本（?name|123|123 加结尾按键）
        Send("{BS " totalBackspaces "}")

        ; 微延迟 20 毫秒缓冲
        Sleep(20)

        ; 4. 转换换行符，并纯靠 SendText 输出
        finalResult := StrReplace(finalResult, "\n", "`n")
        SendText(finalResult)
    }
}
;----------------------------------------------------------------------------------------------------------热字符串class
;----------------------------------------------------------------------------------------------------------日志记录操作cliphis类clas
    ...
    static pastpng2dir()
    {
       try{
           if !(f := ak.getactivePath())
               return
           ; CF_HDROP：资源管理器/桌面自己的 Ctrl+V 已经粘贴文件，这里再 FileCopy 会变成两份
           if DllCall("IsClipboardFormatAvailable", "UInt", 15, "Int")
               return
           ; 纯位图/截图（无文件列表）才另存到当前文件夹
...
class runbox
{
    ...
    static runconfigCmd(cmdstr)
    {
        ...
        if inStr(cmdline,"http://")==1 or  inStr(cmdline,"https://")==1{ ;打开默认浏览器
            try Run cmdline
        }else if FileExist(cmdline){ ;打开文件夹或是exe/文件
            if InStr(FileExist(cmdline), "D") {
                try Run 'explorer.exe "' cmdline '"'
            } else {
                try Run cmdline
                catch {
                    try Run '"' cmdline '"'
...
run_config_C22222:=""
run_config_C22222.=""
run_config_C22222.=""
run_config_C22222.=""
sys_config_C33333:=""
sys_config_C33333.=""
sys_config_C33333.=""
sys_config_C33333.=""
sys_config_C33333.=""
